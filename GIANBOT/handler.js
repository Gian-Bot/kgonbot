            ...
            if (chat.welcome) {
                ...
                for (let user of participants) {
                    ...
                    try {
                        ...
                        text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || '*𝗚𝗿𝘂𝗽𝗼 𝗽𝗶𝗼𝗹𝗮 𝗽🔥*') :
                            (chat.sBye || this.bye || conn.bye || 'Bye, @user!')).replace('@user', '@' + user.split('@')[0])
                            let apii = await this.getFile(pp)
                            this.sendHydrated(id, text, groupMetadata.subject, apii.data,                                                       'https://github.com/Gian-Bot', '𝗞𝗚𝗢𝗡-𝗚𝗜𝗔𝗡', null, null, [
                            [(action == 'add' ? '𝙎𝙚 𝙪𝙣𝙞𝙤' : '𝗦𝗲 𝗳𝘂𝗲 𝗺𝗶 𝗰𝘂𝗹𝗶𝘁𝗼 𝗺𝗮𝗻𝗼😥'), '.s'],    
                            ['🔥 𝙄𝙧 𝙖𝙡 𝙈𝙚𝙣𝙪', '/menu']
                            ], '', { mentions: [user]})
                           }
                }
...
global.dfail = (type, m, conn) => {
    let msg = {
        rowner: '╰⊱⚠️⊱ *𝘼𝘿𝙑𝙀𝙍𝙏𝙀𝙉𝘾𝙄𝘼* ⊱⚠️⊱╮\n\n_*¡¡Este comando solo lo puede usar Mí Creador!!*_',
        owner: '╰⊱⚠️⊱ *𝘼𝘿𝙑𝙀𝙍𝙏𝙀𝙉𝘾𝙄𝘼* ⊱⚠️⊱╮\n\n_*¡¡Este comando solo puede ser utilizado por Mí Creador_',
        mods: '╰⊱⚠️⊱ *𝘼𝘿𝙑𝙀𝙍𝙏𝙀𝙉𝘾𝙄𝘼* ⊱⚠️⊱╮\n\n_*¡¡Este comando solo puede ser utilizado por Moderador(es) y Mí Creador!*_',
        premium: '╰⊱⚠️⊱ *𝘼𝘿𝙑𝙀𝙍𝙏𝙀𝙉𝘾𝙄𝘼* ⊱⚠️⊱╮\n\n_*¡¡Este comando es solo para miembros Premium y Mí Creador*_',
        ...
        unreg: '╰⊱⚠️⊱ *𝘼𝘿𝙑𝙀𝙍𝙏𝙀𝙉𝘾𝙄𝘼* ⊱⚠️⊱╮\n\n_*¡Qué esperas para estar Verificando en KGON Usa el comando #verificar*_',
        restrict: '╰⊱⚠️⊱ *𝘼𝘿𝙑𝙀𝙍𝙏𝙀𝙉𝘾𝙄𝘼* ⊱⚠️⊱╮\n\n_*¡¡Esta función está Restringida | disable por Mí Creador!!*_'
    }[type]
